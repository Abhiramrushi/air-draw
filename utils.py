...
class PerformanceMetrics:
    """Track processing latency and frames per second (FPS)."""
    ...
    def end_frame(self):
        ...
        latency = (now - self.last_time) / cv2.getTickFrequency() * 1000  # ms
        self.latencies.append(latency)
        if len(self.latencies) > self.window_size:
            self.latencies.pop(0)

        avg_latency = sum(self.latencies) / len(self.latencies)
        self.fps = 1000.0 / avg_latency if avg_latency > 0 else 0.0
        return latency, avg_latency, self.fps

    def get_summary(self) -> str:
        if not self.latencies:
            return "Latency: N/A | FPS: N/A"
        avg_latency = sum(self.latencies) / len(self.latencies)
        return (
            f"Latency: {avg_latency:.1f}ms | FPS: {1000.0/avg_latency:.1f}"
            if avg_latency > 0 else "Latency: 0ms | FPS: N/A"
        )
